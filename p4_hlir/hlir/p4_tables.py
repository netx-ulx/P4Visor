...
class p4_conditional_node (p4_node):
    ...
    TODO: docstring
    """
    def __init__ (self, hlir, condition, program_version=0):
        name = "_condition_"+str(len(hlir.p4_conditional_nodes) + program_version)
...
def p4_control_flow_to_table_graph(hlir, call_sequence, program_version=0):
    visited = set()
    # print'ZP: main-016-p4_tables.py self:', pprint(vars(hlir))
    return _p4_control_flow_to_table_graph(hlir, call_sequence,
                                           None, None, visited, program_version=program_version)

def _p4_control_flow_to_table_graph(hlir,
                                    call_sequence, parent_fn,
                                    conditional_barrier,
                                    visited,program_version=0):
    ...
    for call in call_sequence:
        ...
        if type(call) is p4_table:
            ...
            # note: ShadowP4 can also rename table here
...
def optimize_table_graph(hlir):
    ...
    while change:
        change = False
        for _, p4_node in hlir.p4_nodes.items():
            # print 'ZP: p4_tabl 001 p4_node:', pprint(vars(p4_node))
