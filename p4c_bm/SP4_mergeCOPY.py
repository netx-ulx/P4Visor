...
#NEWEST VERSION DUARTE
def merge_header_instances(h_mg, h_r, h_meta):
    #global eq_list = []
    print 'LOG|MERGE|p4_header instances'
    # 1 For each header in r and mg
	...
	if  not header_instR.metadata:
		for header_nameMG, header_instMG in h_mg.p4_header_instances.items():
			# 1.1 See if headerMG is in StrongEquivalenceList or SimpleEquivalenceList
			# if it is, match with next header of MG
			if header_nameMG in h_r.lStrongEq:
				continue
			if header_nameMG in h_r.lSimpleEq:
				continue
			
			# 1.2 Determine equivalence type
		...
		h_mg.p4_header_instances[header_nameR] = header_instR
    h_mg.p4_header_instances.update(h_meta.p4_header_instances)
...
def check_equivalent_headers(header_instMG, header_instR):
	...
	if len(header_instR.fields) == len(header_instMG.fields):
		if header_instMG.name != header_instR:
...
def check_id(h_mg, h_r, header_instMG, header_instR):
    # 1 find state that extracts the header
    foundMG = False
    for _, parse_stateMG in h_mg.p4_parse_states.items():
	if foundMG:
		continue
	if parse_stateMG.call_sequence[0][1] == header_instMG:
		# 2 find state that branches to state from #1
		originMG = iter(parse_stateMG.prev).next()
   		for key, state in originMG.branch_to.items():
			if state == parse_stateMG:
				keyMG = key
				foundMG = True

    for _, parse_stateR in h_r.p4_parse_states.items():
	foundR = False
	if foundR:
		continue
	if parse_stateR.call_sequence[0][1] == header_instR:
		# 2 find state that branches to state from #1
		originR = iter(parse_stateR.prev).next()
   		for key, state in originR.branch_to.items():
			if state == parse_stateR:
				keyR = key
				foundR = True
    # 2.1 compare ids for both headers
    return keyMG == keyR
...
def merge_parser_statesCopy(h_mg, h_r, h_meta):
	...
	if parser_nameR == 'start':
            continue
	found = False
	for parser_nameMG, parser_stateMG in h_mg.p4_parse_states.items():
    		if parser_stateR.return_statement[1][0] == parser_stateMG.return_statement[1][0] \
	  	   and parser_stateR.call_sequence[0][1].name == parser_stateMG.call_sequence[0][1].name:
			found = True
			for key, state in parser_stateR.branch_to.items():
				# If the merged parser state does not have this key-to-state transition, add it
				if parser_stateMG.branch_to.get(key, -1) == -1:
					h_mg.p4_parse_states[parser_nameMG].branch_to[key] = state
			continue

			
	if not found:
		h_mg.p4_parse_states[parser_nameR] = parser_stateR
# break p4c_bm/SP4_merge.py:1317
#NEWEST VERSION MADE BY DUARTE
def merge_parser_states(h_mg, h_r, h_meta):
    ...
    # TODO: add header and parse state at the start 
    #use shadow_id for now
    h_mg.p4_parse_states['parse_shadow_tag'] = h_meta.p4_parse_states['parse_shadow_tag']
 
    branch_case_default = p4_hlir.hlir.p4_parser.P4_DEFAULT
    parse_eth_state = h_mg.p4_parse_states['start'].branch_to[branch_case_default]
    h_mg.p4_parse_states['start'].branch_to.clear()
    h_mg.p4_parse_states['start'].branch_to[branch_case_default] = h_mg.p4_parse_states['parse_shadow_tag']
    
    
    #add select fields and transitions to shadow state, i.e.,
    #select(shadow_tag.shadow_id)
    #ffff : parse_ethernet
    h_mg.p4_parse_states['parse_shadow_tag'].branch_on = []
    h_mg.p4_parse_states['parse_shadow_tag'].branch_on = [h_meta.p4_header_instances['shadow_tag'].fields[0]]
    tempDict = OrderedDict()
    tempDict[0x1111] = parse_eth_state
    h_mg.p4_parse_states['parse_shadow_tag'].branch_to = tempDict
	...
	if parser_nameR == 'start':
            continue
	if parser_nameR == 'parse_ethernet':
		h_mg.p4_parse_states['parse_shadow_tag'].branch_to[0xffff] = parser_stateR
	header_R = ''
	#if the node is purely conditional
	if parser_stateR.call_sequence == []:
		rename_and_add_state(h_r, h_mg, parser_stateR, h_meta)
	...
	if header_R in h_r.lStrongEq:
		share_state(h_r, h_mg, parser_stateR, h_r.lStrongEq[header_R])
	elif header_R in h_r.lSimpleEq:
		share_state(h_r, h_mg, parser_stateR, h_r.lSimpleEq[header_R])
...
def add_shadow_field_to_states(h_mg):
	for _, state in h_mg.p4_parse_states.items():
		if state.name == 'start' or state.name == 'parse_shadow_tag':
			continue
		if state.return_statement[0] != 'immediate':
			state.branch_on.insert(0, h_mg.p4_header_instances['shadow_tag'].fields[0])
		add_select_fields_2(h_mg, state)	

def rename_and_add_state(h_r, h_mg, parse_stateR, h_meta):
	...
	if parse_stateR.return_statement[0] == 'select':
		if parse_stateR.name in h_mg.p4_parse_states:
			...
			add_select_fields(h_r, parse_stateR)
			parse_stateR.return_statement[1].insert(0, 'shadow_tag.shadow_id')
			parse_stateR.branch_on.insert(0, h_meta.p4_header_instances['shadow_tag'].fields[0])
		else:
			add_select_fields(h_r, parse_stateR)
			parse_stateR.return_statement[1].insert(0, 'shadow_tag.shadow_id')
			parse_stateR.branch_on.insert(0, h_meta.p4_header_instances['shadow_tag'].fields[0])
...
def add_select_fields(h_r, parser_stateR):
	if parser_stateR.return_statement[0] == 'select':
		for transition in parser_stateR.return_statement[2]:
			if transition[0][0][0][0] == 'value':
				hexa = hex(transition[0][0][0][1]).split('x')[-1]
				width = parser_stateR.branch_on[0].width
				if width % 4 != 0:
					hex_width = width / 4 + 1
				else:
					hex_width = width / 4
				if len(hexa) < hex_width:
					while len(hexa)<width:
						hexa = '0' + hexa
				# Add id (for now, fake value)
				hexa = 'ffff' + hexa 
				transition[0][0][0][1] = int(hexa, 16)
		newDict = OrderedDict()
		for transition_2 in parser_stateR.branch_to.items():
			if isinstance(transition_2[0], int):				
				if isinstance(parser_stateR.branch_on[0], tuple):
					newDict[transition_2[0]] = transition_2[1]
				else:
					hexa = hex(transition_2[0]).split('x')[-1]
					width = parser_stateR.branch_on[0].width
					if width % 4 != 0:
						hex_width = width / 4 + 1
					else:
						hex_width = width / 4
					if len(hexa) < hex_width:
						while len(hexa) < hex_width:
							hexa = '0' + hexa
					# Add id (for now, fake value)
					hexa = 'ffff' + hexa 
					temp = int(hexa, 16)
					newDict[temp] = transition_2[1]
...
def add_select_fields_2(h_r, parser_stateR):
	if parser_stateR.return_statement[0] == 'select':
		...
		for transition_2 in parser_stateR.branch_to.items():
			if isinstance(transition_2[0], int):
				hexa = hex(transition_2[0]).split('x')[-1]
				width = parser_stateR.branch_on[1].width
				if width % 4 != 0:
					hex_width = width / 4 + 1
				else:
					hex_width = width / 4
	...
	#add transition case before sharing
		...
		parser_stateR.branch_to[default] = parser_stateR.branch_to.pop(default)
			

def add_select_fields_shared(h_r, parser_stateR, state_MG):
	if parser_stateR.return_statement[0] == 'select':
		width = 0
		for field in parser_stateR.branch_on:
			width = field.width + width
...
# break p4c_bm/SP4_merge.py:1385.split('x')[-1]

def share_state(h_r, h_mg, parser_stateR, header_MG):
	...
	for _, stateTemp in h_mg.p4_parse_states.items():
		if len(stateTemp.call_sequence) > 0:
			if stateTemp.call_sequence[0][1].name == header_MG :
				...
				break
	if state_MG != '':
		#TODO: Add different fields, and reorder
		add_select_fields_shared(h_r, parser_stateR, state_MG)

		#add transitions		
		for key, state in parser_stateR.branch_to.items():

				#If the merged parser state does not have this key-to-state transition, add it
				if state_MG.branch_to.get(key, -1) == -1:
		...
		#for state in parser_stateR.prev:
			#for key, tempState in state.branch_to
			#state.branch_to[header_MG.name] = header_MG
